...
a = cell(n_agents,1);
for i = 1:n_agents
  P_mes{i} = 100*eye(states_len*n_agents);
  for j = 1:n_agents
    if i ~= j
      ...
      if dist3D <= agents{i}.Rc
        ...
        P_mes{i}((i-1)*states_len+1:i*states_len,(j-1)*states_len+1:j*states_len) = H*agents{i}.P_est{i}*H' + agents{i}.R_relative;
        P_mes{i}((j-1)*states_len+1:j*states_len,(i-1)*states_len+1:i*states_len) = H*agents{i}.P_est{i}*H' + agents{i}.R_relative;
        ...
        P_mes{i}((i-1)*states_len+1:i*states_len,(j-1)*states_len+1:j*states_len) = P_est_init*eye(states_len, states_len); 
        P_mes{i}((j-1)*states_len+1:j*states_len,(i-1)*states_len+1:i*states_len) = P_est_init*eye(states_len, states_len); 
      end
      ...
      P_mes{i}((i-1)*states_len+1:i*states_len,(j-1)*states_len+1:j*states_len) = agents{i}.R_GPS;
    end
  end 
...
for i = 1:n_agents
  PredP = P_pred{i};
%   P{i} = inv(P_pred{i} + F{i});
%   x_pred{i} = P{i}*(PredP*x_pred{i} + a{i});
